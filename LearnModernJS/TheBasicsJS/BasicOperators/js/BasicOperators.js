...
Остальные арифметические операторы работают только с числами: ${
  ...
  value1 - value2
)}). Строка преобразуется в число в том случае если в строке указано число.`);
...
console.log(`Другой пример. Мы имеем две переменные со значениями (аргументами, операндами) ${value1} и ${value2}. При этом их тип ${typeof value1} и ${typeof value2} соответственно.
  Сложение + : ${value1 + value2} (Тип ${typeof (value1 + value2)});
  Для того чтобы преобразовать эти значение в цифровые и сложить используется унарный оператор + перед каждой переменной +value1 + +value2 (можно использовать Number ()): ${
  +value1 + +value2
} (Тип ${typeof (+value1 + +value2)})`);
...
const number1 = +prompt('Введите первое число', 1);
const number2 = +prompt('Введите второе число', 2);
